...
function $stickyStateRun($stickyState) {
    _StickyState = $stickyState;
}

$stickyStateRun.$inject = ["$stickyState"];

angular.module("ct.ui.router.extras.sticky").run($stickyStateRun);

function $stickyStateConfig($provide, $stateProvider, $stickyStateProvider, $urlMatcherFactoryProvider, uirextras_coreProvider) {
    var core = uirextras_coreProvider;
    ...
    function $stateDecorator ($state, $log, $q) {
    ...
    $stateDecorator.$inject = ['$delegate', '$log', '$q'];
...
$stickyStateConfig.$inject = ['$provide', '$stateProvider', '$stickyStateProvider', '$urlMatcherFactoryProvider', 'uirextras_coreProvider'];

angular.module("ct.ui.router.extras.sticky").config($stickyStateConfig);
...
function $previousStateService ($rootScope, $state, $q) {
...
$previousStateService.$inject = ['$rootScope', '$state', '$q'];

angular
    .module('ct.ui.router.extras.previous', [
...
function $previousStateRun ($previousState) {}

$previousStateRun.$inject = ['$previousState'];

angular.module('ct.ui.router.extras.previous').run($previousStateRun);

angular.module("ct.ui.router.extras.transition", ['ct.ui.router.extras.core'])
    .config($transitionConfig);

$transitionConfig.$inject = ['$provide'];

function $transitionConfig($provide) {
    // Decorate the $state service, so we can replace $state.transitionTo()
    $provide.decorator("$state", $stateDecorator);

    function $stateDecorator($state, $rootScope, $q, $injector) {
    ...
    $stateDecorator.$inject = ['$delegate', '$rootScope', '$q', '$injector'];
